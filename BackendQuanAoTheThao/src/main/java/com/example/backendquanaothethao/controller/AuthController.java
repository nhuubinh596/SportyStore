...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest req) {
        // authService.login trả UserAccount nếu success hoặc null/ném exception nếu fail
    ...
    public ResponseEntity<?> register(@RequestBody RegisterRequest req) {
        ...
        u.setPassword(req.getPassword()); // Lưu pass thô (NoOp)
        u.setName(req.getName());
        u.setEmail(req.getEmail());
        u.setPhone(req.getPhone());
        u.setCreatedAt(java.time.LocalDateTime.now());

        // 3. Cấp quyền USER (Tìm ID = 2 hoặc 1 tùy DB của bạn, thường USER là ID 2)
        // Mẹo: Tìm theo tên cho chắc ăn
        Role roleUser = roleRepository.findByName("ROLE_USER").orElse(null);
        if (roleUser == null) {
            // Fallback nếu chưa có role trong DB thì lấy tạm role đầu tiên
